import { useEffect, useRef } from "react";
import { useAuth } from "@/context/AuthContext";
import { useNavigate } from "react-router-dom";
import { toast } from "sonner";
import { useTranslation } from "react-i18next";
import { Button } from "@/components/ui/button";

interface GoogleAuthButtonProps {
  text?: "signin_with" | "signup_with" | "continue_with";
  className?: string;
}

export function GoogleAuthButton({ text = "continue_with", className = "w-full" }: GoogleAuthButtonProps) {
  const { googleLogin, user } = useAuth();
  const navigate = useNavigate();
  const { t, i18n } = useTranslation();
  const buttonRef = useRef<HTMLDivElement>(null);

  const clientId = import.meta.env.VITE_GOOGLE_CLIENT_ID || "";

  const buttonLabel =
    text === "signup_with"
      ? t("auth.signUpWithGoogle")
      : text === "signin_with"
      ? t("auth.signInWithGoogle")
      : t("auth.continueWithGoogle");

  useEffect(() => {
    if (!clientId) return;

    // Load Google Identity Services script
    const scriptId = "google-identity-script";
    let script = document.getElementById(scriptId) as HTMLScriptElement | null;

    const handleGoogleCallback = async (response: any) => {
      if (!response?.credential) {
        toast.error("Google authentication failed. No credential received.");
        return;
      }

      try {
        if (googleLogin) {
          await googleLogin(response.credential);
          toast.success(t("auth.loginSuccess") || "Successfully signed in with Google!");

          const stored = localStorage.getItem("userData");
          const role = stored ? JSON.parse(stored)?.role : user?.role;

          const searchParams = new URLSearchParams(window.location.search);
          const redirect = searchParams.get("redirect");
          const planId = searchParams.get("planId");

          if (role === "admin") navigate("/admin");
          else if (role === "mentor" || role === "tutor") navigate("/mentor");
          else if (redirect) {
            navigate(`${redirect}${planId ? `?planId=${encodeURIComponent(planId)}&newOrder=true` : ""}`);
          } else {
            navigate("/");
          }
        }
      } catch (err: any) {
        toast.error(err.message || "Google authentication failed. Please try again.");
      }
    };

    const initializeGoogle = () => {
      // @ts-ignore
      if (window.google?.accounts?.id && buttonRef.current) {
        // @ts-ignore
        window.google.accounts.id.initialize({
          client_id: clientId,
          callback: handleGoogleCallback,
          auto_select: false,
          cancel_on_tap_outside: true,
        });

        // @ts-ignore
        window.google.accounts.id.renderButton(buttonRef.current, {
          type: "standard",
          theme: "outline",
          size: "large",
          text: text,
          shape: "rectangular",
          logo_alignment: "left",
          width: buttonRef.current.offsetWidth || 380,
          locale: i18n.language === "kz" ? "kk" : i18n.language,
        });
      }
    };

    if (!script) {
      script = document.createElement("script");
      script.id = scriptId;
      script.src = "https://accounts.google.com/gsi/client";
      script.async = true;
      script.defer = true;
      script.onload = initializeGoogle;
      document.body.appendChild(script);
    } else {
      initializeGoogle();
    }
  }, [clientId, googleLogin, navigate, t, text, user, i18n.language]);

  const handleManualClick = () => {
    if (!clientId) {
      toast.error("Google Client ID is not configured yet. Please add VITE_GOOGLE_CLIENT_ID to .env");
      return;
    }

    // @ts-ignore
    if (window.google?.accounts?.id) {
      // @ts-ignore
      window.google.accounts.id.prompt();
    } else {
      toast.error("Google Sign-In is initializing. Please try again in a moment.");
    }
  };

  return (
    <div className={className}>
      {clientId ? (
        <div ref={buttonRef} className="w-full flex justify-center min-h-[44px]" />
      ) : (
        <Button
          type="button"
          variant="outline"
          onClick={handleManualClick}
          className="w-full h-10 rounded-lg flex items-center justify-center gap-3 border-border/80 hover:bg-muted/50 text-foreground font-medium"
        >
          <svg className="h-4 w-4 shrink-0" viewBox="0 0 24 24">
            <path
              fill="#4285F4"
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
            />
            <path
              fill="#34A853"
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
            />
            <path
              fill="#FBBC05"
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z"
            />
            <path
              fill="#EA4335"
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z"
            />
          </svg>
          <span>{buttonLabel}</span>
        </Button>
      )}
    </div>
  );
}
