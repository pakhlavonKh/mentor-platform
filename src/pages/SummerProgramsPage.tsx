...
export default function SummerProgramsPage() {
  ...
  const summerPrograms = useMemo(() => {
    return grants.filter((g) => g.type === "summer_program");
  }, [grants]);

  const countries = useMemo(() => [...new Set(summerPrograms.map((g) => g.country))], [summerPrograms]);

  const filtered = useMemo(() => {
    return summerPrograms.filter((g) => {
  ...
  }, [summerPrograms, search, country, funding, lt]);

  return (
    <PageLayout>
      <div className="max-w-6xl mx-auto space-y-6">
        <div>
          <h1 className="font-display text-3xl font-bold text-foreground">{t("summerPrograms.title")}</h1>
          <p className="text-muted-foreground mt-1">{t("summerPrograms.description")}</p>
        </div>
