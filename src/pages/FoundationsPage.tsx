...
export default function FoundationsPage() {
  ...
  const foundations = useMemo(() => {
    return grants.filter((g) => g.type === "foundation");
  }, [grants]);

  const countries = useMemo(() => [...new Set(foundations.map((g) => g.country))], [foundations]);

  const filtered = useMemo(() => {
    return foundations.filter((g) => {
  ...
  }, [foundations, search, country, funding, lt]);

  return (
    <PageLayout>
      <div className="max-w-6xl mx-auto space-y-6">
        <div>
          <h1 className="font-display text-3xl font-bold text-foreground">{t("foundations.title")}</h1>
          <p className="text-muted-foreground mt-1">{t("foundations.description")}</p>
        </div>
