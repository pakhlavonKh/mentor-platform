...
import { User, Mail, Globe, BookOpen, GraduationCap, Bookmark, FileCheck, CreditCard, Clock, CheckCircle2, LogOut, ArrowRight } from "lucide-react";
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    api.grants.list({ limit: "2" }).then((res) => setGrants(res.data)).catch(() => {});
  }, []);
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { icon: <Bookmark className="h-5 w-5" />, label: t("profile.savedGrants"), value: "2" },
              { icon: <FileCheck className="h-5 w-5" />, label: t("profile.submissions"), value: "0" },
              { icon: <CreditCard className="h-5 w-5" />, label: t("profile.documentReviews"), value: "0" },
        ...
        <Button variant="outline" className="w-full rounded-full">{t("common.logout")}</Button>
      </div>
    </PageLayout>
  );
}
